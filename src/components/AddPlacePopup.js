...
function AddPlacePopup(props) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    console.log(namePlaceRef.current.value, linkPlaceRef.current.value)
    props.onSubmitPlace(namePlaceRef.current.value, linkPlaceRef.current.value);
  }

  return (
    <PopupWithForm id={"popup-add"} title={"Новое место"} isOpen={props.isOpen} onClose={props.onClose}>
      <form className="popup__form" name="form-add" id="popupAddForm" onSubmit={handleSubmit} noValidate>
